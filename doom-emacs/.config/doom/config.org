...
#+end_src
** Latex Preview
This allows the backend for dvipng to produce latex equations directly in org mode files 
Please ntoe that you need to have installed: ~texlive-latexrecommended texlive-latexextra texlive-fontsrecommended texlive-fontsextra texlive-plaingeneric~ (Arch linux repos)
#+begin_src elisp
...
** Task Management
*** Task States
In org mode it is possible to customize the "TODO" keywords, and also define custom todo workflow.
In this snippet I will define some custom todo items / todo states: 
#+begin_src elisp
...
#+end_src
** Latex Header blocks
This part of the config is to setup latex with header packages, ignore headlines are a package that allows to introduce trees of the document that are not exported simply by using org-mode tags.
Such tags can be :noexport: which prevents the entire tree and subtree not to be exported into the pdf (normally would put all the configuration part of the doc) :ignore instead allows the content of a tree or subtree to be exported but without the heading
#+begin_src elisp
(after! org
...
#+end_src
In the following I setup the global latex configuration for Latex export
#+begin_src elisp
(after! org
