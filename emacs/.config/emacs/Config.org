...
#+end_src
** Touch-friendly UI on Android
minimal-emacs.d disables the menu bar, tool bar, and context menu everywhere
by default (they cost a startup redraw and desktop workflows here are
keyboard-driven).  On a touchscreen those are exactly the affordances that
make commands discoverable without memorizing keybindings, so re-enable
them for Android only.  This must run in pre-early-init.el: it sets
=minimal-emacs-ui-features= before early-init.el reads it, and =glz/platform=
(defined later, in post-init.el) doesn't exist yet at this point in startup.
#+begin_src emacs-lisp
...
| Module          | Enabled on                                  |
|-----------------+----------------------------------------------|
| org-roam        | linux, android (only if built-in sqlite works) |
| org-roam-ui     | linux                                        |
| forge           | linux                                        |
| nix-mode        | linux                                        |
| testfall        | windows                                      |
...
| canape-par      | windows                                      |
| lsp-c           | windows, linux                               |
| latex-preview   | windows, linux                               |
| open-externally | windows, linux                               |
| magit           | windows, linux                               |
| pdf-tools       | windows, linux                               |

Android gets none of the git-workflow tooling (magit/ediff/smerge — not
realistic to commit from a phone), no clangd LSP, PDF, or LaTeX-preview
native-compile dependencies, and no OS-shell-out for external files.

Org Roam is the one exception worth enabling on a phone — reading notes is
the main Android use case — but its database needs SQLite, and the only
fallback for a build without one compiled in is a native module that needs
a C compiler at runtime, which a packaged Android app won't have. So probe
for it instead of assuming: enable Org Roam on Android only when
=(sqlite-available-p)= is true. When it isn't, =SPC o r= still gets you to
your notes — it just opens the roam directory in Dired instead of the
fuzzy node finder (see the Org Roam section). org-roam-ui (the graph
viewer, served over a local webserver you'd open in a browser) stays
desktop-only regardless, sqlite or not — not a touch workflow.

See the packages below for what Android gets instead (touch-friendly
menu/tool bar, corfu completion, org, dired, org-capture, etc. all stay on
for every platform).
...
A phone screen has no physical Ctrl/Meta/Esc keys, and Meow's leader (=SPC=)
covers most everyday commands without needing them — but a few things (org
tables, capture's =C-c C-c= to finish, isearch) still expect a real
modifier. =modifier-bar-mode= is Emacs's own built-in answer: it adds a
small second tool bar with Ctrl/Meta/Shift/etc buttons that modify the next
keypress and pop the on-screen keyboard up (built into Emacs 30+, no extra
app). It piggybacks on the tool bar already re-enabled for Android above.

If you type enough to want persistent modifier keys instead of tapping a
button each time, /Hacker's Keyboard/ (F-Droid/Play Store) is the
long-standing recommendation in the Emacs-on-Android community — it has a
dedicated Ctrl/Alt/Esc/Tab row, at the cost of no glide typing. For a
Bluetooth/USB keyboard, /External Keyboard Helper/ fixes up modifier keys
Android otherwise mishandles.

Also: =pixel-scroll-precision-mode= makes touch-drag scrolling track your
finger instead of jumping by whole lines, and the default face size is
bumped up ~15% — desktop-sized text reads small on a phone screen. This
scales whatever font the Android build already picked rather than naming
one, since font availability there isn't something to guess at from a
desktop checkout; fine-tune further on-device with the toggle map's
=text-scale-adjust= binding (SPC t, then the equals key, then =+=/=-=/=0=).
#+begin_src emacs-lisp
...
#+end_src
*** Mouse horizontal scroll (Logitech MX Master thumb wheel)
#+begin_src emacs-lisp
(use-package mwheel
  :ensure nil
  :custom
...
#+end_src
**** Marginalia
=package.el= may byte-compile marginalia before compat is loaded, causing the
=compat-call= macro to resolve to the native =seconds-to-string= (1 arg on
Emacs <31) instead of the compat polyfill.  Delete the miscompiled =.elc= so
the =.el= source is used — the macro then expands correctly at load time.
#+begin_src emacs-lisp
...
#+end_src
**** Performance settings for large generated org files
Use text-properties for folding (O(1) cursor movement vs O(n) for overlays).
For the generated =requirements.org= file, also disable expensive display
features that are unnecessary for a read-only reference document.
#+begin_src emacs-lisp
...
#+end_src
**** AUCTeX-based LaTeX preview (org-auctex)
Org's built-in LaTeX preview (=dvipng=, synchronous) recompiles the whole
LaTeX preamble on every preview, which is the main source of slowness on
notes with many equations. AUCTeX's =preview-latex= avoids that by caching
a precompiled preamble format dump, so only the fast body pass reruns.
=org-auctex= wires that engine into Org, replacing the =org-latex-preview=
binding (=C-c C-x C-l=) with its own async-friendly version while the minor
mode is on.

AUCTeX native-compiles ~380 elisp files on install and needs a real TeX
toolchain (=latex=, =dvisvgm=) on =PATH= — neither is realistic on a phone
build of Emacs, so this is skipped on Android like the other native-compile
/ external-toolchain modules (see Feature Flags).
#+begin_src emacs-lisp
...
#+end_src
**** Directories, task management and capture templates
Consolidated into the =use-package org= block above under =**** Basic settings and appearance=.
**** Org Roam
#+begin_src emacs-lisp
...
Serves the graph over a local webserver and opens it in a browser —
desktop-only, so it's gated separately from Org Roam itself (which is also
enabled on Android when possible; see Feature Flags).
#+begin_src emacs-lisp
(when glz/enable-org-roam-ui
...
#+end_src
*** Ediff settings
Ediff uses =run-hooks= instead of =run-mode-hooks= in its control buffer, so
=meow-global-mode= never sees it.  We manually enable meow in motion state
there so the SPC leader still works.  Ediff's own single-key commands (n/p/a/b/q)
fall through because they are not bound in =meow-motion-map=.  Diff buffers are
also put in motion state during the session and restored to normal on quit.
This whole block only makes sense as part of a git/magit-centric diff
workflow, so it is skipped wherever magit is (see =glz/enable-magit=).
#+begin_src emacs-lisp
...
#+end_src
*** Magit (git client)
Skipped on Android via =glz/enable-magit= — not a realistic place to commit from.
#+begin_src emacs-lisp
(when glz/enable-magit
(use-package magit
...
#+end_src
*** Project.el — magit instead of VC-dir
#+begin_src emacs-lisp
(use-package project
  :ensure nil
  :custom
...
#+end_src
*** Smerge — inline conflict resolution
Resolves Git merge conflicts inline in the file buffer (no split windows).
Navigate conflicts, inspect each side with word-level diff highlighting, keep
one side or auto-resolve, and fall back to ediff for tricky cases.

Keybindings live on =SPC g m= (git → merge):

| Key | Action |
|-----+--------|
| =n= / =p= | next / previous conflict |
| =u= | keep upper (ours, =<<<<<<<= block) |
| =l= | keep lower (theirs, =>>>>>>>= block) |
| =b= | keep base (common ancestor, 3-way only) |
| =a= | keep all (concatenate both sides) |
| =r= | auto-resolve (simple cases) |
| =f= | refine — show word-level diffs |
| =e= | open current conflict in ediff |
| =t= | toggle smerge-mode |

Skipped on Android via =glz/enable-magit= along with magit and ediff — resolving
Git conflicts from a phone isn't a realistic workflow either.
#+begin_src emacs-lisp
(when glz/enable-magit
...
#+end_src
*** Markdown mode
Editing and live preview for Markdown files.  =markdown-preview-mode= starts a
local websocket server and opens the rendered document in the default browser;
changes are pushed live on every save (or keystroke with =markdown-preview-delay=).

Requires no external markdown processor — =markdown-preview-mode= bundles its
own.  If you have =pandoc= or =multimarkdown= installed, set
=markdown-command= for richer rendering (e.g. tables, footnotes).
#+begin_src emacs-lisp
...
#+end_src
*** Open binary files with OS default application
Files whose extension matches =glz/external-file-extensions= are handed off
to the OS default application (Explorer/xdg-open) and the Emacs buffer is
immediately discarded.  Extend the list to taste — or shrink it to remove
types you prefer to handle inside Emacs (e.g. =pdf= if you use =pdf-tools=).
...
Generic completion UI driven by whatever =completion-at-point-function= the
current buffer has (Elisp, Eshell history, and — where enabled below — eglot).
Useful on every platform, including Android, independently of the C/LSP setup.
#+begin_src emacs-lisp
(use-package corfu
  :custom
  (corfu-auto        t)
...
#+end_src
*** C / LSP (eglot + clangd)
Requires =clangd= on =PATH= (part of LLVM/Clang).  On Windows install via
=winget install LLVM.LLVM= or the LLVM installer; on Linux install the
=clangd= package.  A =compile_commands.json= (generated by CMake with
=-DCMAKE_EXPORT_COMPILE_COMMANDS=ON= or by Bear) in the project root gives
clangd full cross-file awareness.  Skipped on Android — no realistic C/C++
dev workflow there, and clangd wouldn't be on =PATH= anyway.
#+begin_src emacs-lisp
