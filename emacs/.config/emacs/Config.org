...
realistic to commit from a phone), no clangd LSP or PDF native-compile
dependencies, and no OS-shell-out for external files; see the packages
below for what it gets instead (touch-friendly menu/tool bar, corfu
completion, org, dired, etc. all stay on for every platform).
